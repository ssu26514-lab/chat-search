...
final class SemanticCardScanner {
    ...
    static final class Group {
        ...
        int duplicateCopies() {
            return type == GroupType.EXACT_CONTENT ? Math.max(0, cards.size() - 1) : 0;
        }
    }
    ...
    static final class ScanResult {
        ...
        ScanResult(int totalPng, int parsedCards, int failedCards, long elapsedMs,
    ...
    ScanResult scan(Uri treeUri) throws Exception {
        ...
        List<SemanticCardParser.CardRecord> seeds = enumeratePng(treeUri);
        ...
        for (int i = 0; i < seeds.size(); i++) {
            ...
            progress("正在解析角色卡有效内容：" + (i + 1) + " / " + seeds.size()
                    + "\n" + seed.path);
            try {
        ...
        sortGroups(exact);
        sortGroups(variants);
        progress("分析完成：有效内容相同 " + exact.size() + " 组，存在差异 "
                + variants.size() + " 组");
        return new ScanResult(seeds.size(), parsed.size(), failures.size(),
                System.currentTimeMillis() - started, exact, variants, failures);
    }

    private List<SemanticCardParser.CardRecord> enumeratePng(Uri treeUri) throws Exception {
    ...
    private static List<Group> buildExactGroups(
        ...
        Map<String, List<SemanticCardParser.CardRecord>> byHash = new LinkedHashMap<>();
        for (SemanticCardParser.CardRecord card : cards) {
            byHash.computeIfAbsent(card.functionalHash, ignored -> new ArrayList<>())
                    .add(card);
        }
        List<Group> groups = new ArrayList<>();
        for (Map.Entry<String, List<SemanticCardParser.CardRecord>> entry : byHash.entrySet()) {
    ...
    private static List<Group> buildVariantGroups(
            List<SemanticCardParser.CardRecord> cards) {
        Map<String, List<SemanticCardParser.CardRecord>> byName = new LinkedHashMap<>();
        for (SemanticCardParser.CardRecord card : cards) {
            String name = card.normalizedName();
            if (name.isEmpty() || "未命名".equals(name) || "unknown".equals(name)) continue;
            byName.computeIfAbsent(name, ignored -> new ArrayList<>()).add(card);
        }
        List<Group> groups = new ArrayList<>();
        for (Map.Entry<String, List<SemanticCardParser.CardRecord>> entry : byName.entrySet()) {
            List<SemanticCardParser.CardRecord> members = entry.getValue();
            if (members.size() < 2) continue;
            Set<String> hashes = new HashSet<>();
            for (SemanticCardParser.CardRecord card : members) hashes.add(card.functionalHash);
    ...
    private static String bestTitle(List<SemanticCardParser.CardRecord> cards) {
        ...
        for (SemanticCardParser.CardRecord card : cards) {
            String name = card.characterName == null ? "" : card.characterName.trim();
